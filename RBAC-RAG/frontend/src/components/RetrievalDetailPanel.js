  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="mt-3 rounded-xl border border-white/10 bg-black/20 overflow-hidden">
      ...
      >
        <button
          type="button"
          className="w-full flex items-center justify-between gap-3 px-3 py-2 hover:bg-white/[0.03] transition-colors"
        >
          <div className="flex items-center gap-2">
            <ChevronRight
              className={cn('w-3.5 h-3.5 text-slate-400 transition-transform', open && 'rotate-90')}
              strokeWidth={2}
            />
          ...
          <div className="flex items-center gap-2">
            <span
              ...
              className="inline-flex items-center gap-1 rounded-full border border-emerald-400/25 bg-emerald-500/10 px-2 py-0.5 text-[11px] font-mono text-emerald-200"
            >
      ...
      <CollapsibleContent className="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0">
              ...
              <ul className="space-y-1.5">
                {retrieved.map((r, i) => (
                  <li
                    ...
                    className="flex items-start gap-2 rounded-md bg-white/[0.02] border border-white/6 px-2 py-1.5"
                  >
              ...
              <ul className="space-y-1.5">
                {blocked.map((b, i) => (
                  <li
                    ...
                    className="flex items-start gap-2 rounded-md bg-red-500/[0.04] border border-red-400/15 px-2 py-1.5"
                  >
        ...
        {detail.role && (
          <div className="px-3 pb-2 text-[10px] font-mono text-slate-500">
